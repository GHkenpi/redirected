...
public class speed10 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Raycast用
        Vector3 RayPosition = new Vector3(HMDPosition.x, HMDPosition.y + 3, HMDPosition.z);
        RaycastHit hit;
        if (Physics.Raycast(RayPosition, Vector3.down, out hit))
        {
            ...
            if (HMDPosition.x > 0)
            {
                Vector3 newPosition = new Vector3(HMDPosition.x * cos, HMDPosition.y + groundY, HMDPosition.z);
    ...
    void StartRecording()
    {
        ...
        string fileName = $"{gradient},{result},1.0.csv";
