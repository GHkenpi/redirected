...
public class pitch3 : MonoBehaviour
{
    //HMDの位置座標格納用
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(HMDPosition, Vector3.down, out hit))
        {
            ...
            if (HMDPosition.x < 0 && HMDPosition.x > -0.05)
            {
                origintime = Time.time;
                this.transform.rotation = HMDRotationQ;
            }
            else if (HMDPosition.x > 0)
            {
                if (addRotation > -gradient)
                {
                    addRotation = -6 * (Time.time - origintime);
                }
                else
                {
                    addRotation = -gradient;
                }
                //Debug.Log("time:" + (Time.time - origintime));
                Debug.Log("rot:" + addRotation);
                Debug.Log("gradient:" + gradient);
                // cameraの位置を更新
                //this.transform.position = newPosition;
                Quaternion newRotation = Quaternion.Euler(HMDRotation.x + addRotation, HMDRotation.y, HMDRotation.z);
