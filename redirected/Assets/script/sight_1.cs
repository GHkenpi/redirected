...
public class sight_1 : MonoBehaviour
{
    //HMDの位置座標格納用
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(HMDPosition, Vector3.down, out hit))
        {
            ...
            {
                // 新しい位置を計算
                Vector3 newPosition = new Vector3(HMDPosition.x * cos * 0.9f + 3, HMDPosition.y + (groundY * 0.4f), HMDPosition.z);
                this.transform.position = newPosition;
            }
            else
            {
                Vector3 newPosition = new Vector3(HMDPosition.x + 3, HMDPosition.y, HMDPosition.z);
                this.transform.position = newPosition;
            }
        }
    }
